...
export function Serialize(message) {
  switch (message.type) {
    ...
    case MessageType.CREATE_CONNECTION: {
      const header = Buffer.from([MessageType.CREATE_CONNECTION, (message.mapId >> 8) & 0xFF, message.mapId & 0xFF])
      return header
    }
    case MessageType.CLOSE_CONNECTION: {
      const header = Buffer.from([MessageType.CLOSE_CONNECTION, (message.mapId >> 8) & 0xFF, message.mapId & 0xFF])
      return header
    }
    case MessageType.DATA: {
